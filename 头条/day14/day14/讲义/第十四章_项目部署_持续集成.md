...
2. 解压安装包
   ...
   ![image-20201226192524282](assets/image-20201226192524282.png)
...
3. 配置
   ...
   ==建议大家要手动上传本地仓库到linux中，反之有可能会因为网速下载不了==
...
###  3.1 Jenkins工具配置(jenkins图形化界面中操作)
...
```shell
docker run --name seata --restart=always -p 8091:8091 -e SEATA_IP=192.168.200.131 -e SEATA_PORT=8091 -d seataio/seata-server:1.3.0
```
...
# kafka地址
kafka.servers=192.168.200.129:9092
#nacos地址
nacos.server=192.168.200.129:8848

#数据库连接配置信息
datasource.url=jdbc:mysql://192.168.200.129:33066/leadnews_admin?useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC
datasource.username=root
datasource.password=root

#es配置信息
elasticsearch.host=192.168.200.129
elasticsearch.port=9200

#图片访问ip
fdfs.url: http://192.168.200.129/

#xxljob连接配置信息
xxljob.admin.addresses=http://192.168.200.129:8888/xxl-job-admin
xxljob.admin.appname=leadnews-admin-executor
xxljob.admin.port=9999
```
...
==注意：加载dev的环境的话连接的是130服务器的环境，而加载prod的环境的话，连接是的129服务器的环境==

==注意：每个微服务下都要放置maven_test.proerpties,maven_dev.properties,maven_prop.properties文件==

### 3.4 服务集成Docker配置(windows上黑马头条代码中操作)

每个微服务使用的dockerfile的方式进行构建镜像后创建容器，需要在每个微服务中添加docker相关的配置

（1）修改**每个微服务**的pom文件，添加dockerfile的插件

这里以leadnews-admin,leadnews-admin-gateway,leadnews-user这三个服务为例

==注意：==

dockerfile-maven-plugins这个依赖不太好下载，所以

第一种方式：把资料中的spotify.zip包解压覆盖到本地仓库中的com.spotify包中

第二种方式：使用手动安装依赖脚本命令执行dockerfile-maven-plugins-1.3.6.jar安装到本地仓库中

![image-20201228002239093](assets/image-20201228002239093.png)
...
​		3、通过jenkins去git仓库中拉取代码中依赖

#### 1、把当前黑马头条的项目代码进行打包

第一步，在所有的服务中都要加入这三个properties文件，哪怕是空的都行

![image-20201228003645019](assets/image-20201228003645019.png)

第二步，在所有的微服务中的pom.xml中加入(==注意：除了apis,common.utils.model，seata这些服务不要加==)
...
![image-20201228011552384](assets/image-20201228011552384.png)

那是因为创建的仓库中不是一个全新的仓库，仓库中有文件导致的

![image-20201228011642860](assets/image-20201228011642860.png)
...
![image-20201228011906679](assets/image-20201228011906679.png)

然后：

1、在settings---version control中清空历史记录

![image-20201228012336453](assets/image-20201228012336453.png)

2、重新 创建git本地仓库，然后提交git
